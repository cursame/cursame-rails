...
AutoHtml.add_filter(:image).with({:alt => 'Image', :style =>"max-width:440px;"}) do |text, options|
  ...
  style= options[:style]
  text.gsub(/https?:\/\/.+?\.(jpg|jpeg|bmp|gif|png)(\?\S+)?/i) do |match|
    #r.render("![#{alt}, #{style}](#{match})")
     %{<img src="#{match}" alt="#{alt}" style="#{style}"/>}
  end
end
...
AutoHtml.add_filter(:youtube).with(:width => 420, :height => 315, :frameborder => 0, :wmode => 'transparent') do |text, options|
  ...
  text.gsub(regex) do
    ...
    %{<iframe width="#{width}" height="#{height}" src="#{src}?wmode=transparent" frameborder="#{frameborder}" allowfullscreen></iframe>}
  end
end
