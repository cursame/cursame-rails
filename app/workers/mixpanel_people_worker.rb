...
class MixpanelPeopleWorker
    ...
    def perform(distinct_id, properties, new_profile=true)
        tracker = mixpanel()
        if new_profile then
            puts "\e[1;31m[DEBUG]\e[0m creating mixpanel people profile \e[1;34m=>\e[0m user_id: #{distinct_id}, properties: #{properties}"
            tracker.people.set distinct_id, properties
        else
            tracker.people.append distinct_id, properties
            puts "\e[1;31m[DEBUG]\e[0m updating mixpanel people profile \e[1;34m=>\e[0m user_id: #{distinct_id}, properties: #{properties}"
        end
    end
