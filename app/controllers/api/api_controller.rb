class Api::ApiController < ApplicationController
  ...
  def publications
    case params[:type]
      when 'Course'
        @publications = Course.find(params[:publicacionId]).walls.select("DISTINCT(publication_id,publication_type), walls.*").order('created_at DESC').paginate(:per_page => params[:limit].to_i, :page => params[:page].to_i)
      else
        @publications = @network.walls.select("DISTINCT(publication_id,publication_type), walls.*").order('created_at DESC').paginate(:per_page => params[:limit].to_i, :page => params[:page].to_i)
    end
