...
<div id="pageless-loader" style="display:none;text-align:center;width:100%;">\
  ...
  var init = function (opts) {
    ...
    settings.onInit.call(undefined,settings);
    // start the listener
    startListener();
  };
  ...
  // * bind a scroll event
  // * trigger this once in case of reload
  var startListener = function() {
    $container.bind(SCROLL+' '+RESIZE, watch)
              .trigger(SCROLL);
  };
  //para poder lanzar de nuevo el listener del scroll
  settings.startListener = startListener;
  var watch = function() {
    ...
    if(!isLoading && (distanceToBottom() < settings.distance)) {
      ...
      if (settings.method == 'post') {
        // settings.beforeRequest.call(undefined,settings);
        $.post( settings.url
             , settings.params
             , function (data) {
               ...
               // if there is a complete callback we call it
                if (settings.complete) settings.complete.call(undefined,settings);
            }, 'html');
      } else {
        // settings.beforeRequest.call(undefined,settings);
        $.get( settings.url
